...
import { Transaction, TransactionDetail, Client, Material } from '@prisma/client'
...
export function generateTransactionPDF(transaction: TransactionWithDetails): Blob {
  // 1/4 de hoja carta = 105mm x 279mm (aprox)
  // Convertimos a puntos: 105mm = 298pt, 279mm = 791pt
  const doc = new jsPDF({
...
export function downloadTransactionPDF(transaction: TransactionWithDetails) {
  const pdfBlob = generateTransactionPDF(transaction)
  const url = URL.createObjectURL(pdfBlob)
  const link = document.createElement('a')
  link.href = url
  link.download = `transaccion-${transaction.folio}.pdf`
